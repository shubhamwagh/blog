...
- **A Makefile** that wraps the underlying commands (`k3sup install`/`join` for k3s, the Helm invocations for Cilium, the `flux bootstrap` flow, and so on) into coherent tasks. In this series you ran the raw commands; in HomeOps they're organized behind targets so day-to-day operations are one command instead of five.
...
- **A CrowdSec + Vaultwarden pairing** — CrowdSec reads Traefik logs for abuse patterns, and Vaultwarden (Bitwarden-compatible) stores the human-access credentials that HomeOps's secret tooling surfaces, as the durable second copy of stuff that would otherwise live in a gitignored local file.
